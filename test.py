...
def trajectories_from_video(path, vis_flow=False, vis_trajectories=False,
    ...
    with open(output_file, 'w') as csv_file:
        csv_file.write("trajectory_id,frame,x,y\n")
        for idx, trajectory in enumerate(final_trajectories):
            for frame_num, (x, y) in enumerate(trajectory.coords):
                csv_file.write(f"{idx},{frame_num},{x},{y}\n")
...
path = r'dense-trajectories-action-recognition/data/Extracted/4_2.avi'
trajectories_from_video(path, vis_flow=True, vis_trajectories=True , L = 15 , W = 5 , static_displacement_thresh=1,  
                            max_single_displacement=2)